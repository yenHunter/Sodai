        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <select class="form-select @error('user_id') is-invalid @enderror" id="customerSelect" name="user_id" required
                        data-search-url="{{ route('admin.ecommerce.order.customers.search') }}"
                        data-address-url-template="{{ route('admin.ecommerce.order.customers.address', ['customer' => '__ID__']) }}">
                        @if ($isEdit && $order->user)
                            <option value="{{ $order->user->id }}" selected>{{ $order->user->name }} ({{ $order->user->email }})</option>
                        @endif
                    </select>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control @error('shipping_name') is-invalid @enderror" name="shipping_name"
                            id="shippingName" value="{{ old('shipping_name', $order->shipping_name ?? '') }}" required>
                        @error('shipping_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Email <span class="text-danger">*</span></label>
                        <input type="email" class="form-control @error('shipping_email') is-invalid @enderror" name="shipping_email"
                            id="shippingEmail" value="{{ old('shipping_email', $order->shipping_email ?? '') }}" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control @error('shipping_phone') is-invalid @enderror" name="shipping_phone"
                            id="shippingPhone" value="{{ old('shipping_phone', $order->shipping_phone ?? '') }}" required>
                        @error('shipping_phone')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Country <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('shipping_country') is-invalid @enderror" name="shipping_country"
                            id="shippingCountry" value="{{ old('shipping_country', $order->shipping_country ?? 'Bangladesh') }}" required>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control @error('shipping_city') is-invalid @enderror" name="shipping_city"
                            id="shippingCity" value="{{ old('shipping_city', $order->shipping_city ?? '') }}" required>
                        @error('shipping_city')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">State <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('shipping_state') is-invalid @enderror" name="shipping_state"
                            id="shippingState" value="{{ old('shipping_state', $order->shipping_state ?? '') }}" required>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control @error('shipping_zip') is-invalid @enderror" name="shipping_zip"
                            id="shippingZip" value="{{ old('shipping_zip', $order->shipping_zip ?? '') }}" required>
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('discount_amount') is-invalid @enderror" name="discount_amount"
                        id="discountAmount" min="0" step="0.01" value="{{ old('discount_amount', $order->discount_amount ?? 0) }}">
                    @error('discount_amount')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <div class="mb-3">
                    <label class="form-label">Shipping Charge ($)</label>
                    <input type="number" class="form-control @error('shipping_charge') is-invalid @enderror" name="shipping_charge"
                        id="shippingCharge" min="0" step="0.01" value="{{ old('shipping_charge', $order->shipping_charge ?? 0) }}">
                    @error('shipping_charge')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <div class="mb-3">
                    <label class="form-label">Tax ($)</label>
                    <input type="number" class="form-control @error('tax_amount') is-invalid @enderror" name="tax_amount"
                        id="taxAmount" min="0" step="0.01" value="{{ old('tax_amount', $order->tax_amount ?? 0) }}">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p class="text-muted small mb-0">A temporary password will be generated. The customer can reset it later via "Forgot Password" once login is available.</p>
            </div>
